...
app.use(cors({
  origin: 'https://multiplayer-online-chess-game.vercel.app',  // Restrict to your client URL
  methods: ['GET', 'POST'],
}));
...
io.on('connection', (socket) => {
  ...
  socket.on('offerDraw', ({ room ,  username }) => {
    socket.to(room).emit("drawOffer", { username });
  });
  
  socket.on('acceptDraw', ({ room }) => {
    io.to(room).emit('drawAccepted');
  });

  socket.on("declineDraw", ({ room }) => {
    io.in(room).emit("drawDeclined");
  });

  socket.on('resign', ({ room, username, winner }) => {
    io.to(room).emit('resign', { username, winner });
  });
